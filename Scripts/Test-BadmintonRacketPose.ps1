param([Parameter(Mandatory=$true)][string]$GameExecutable,[switch]$WheelAngles)
...
if($WheelAngles){$taskArgs+='-BadmintonWheelPoseTest'}
...
try {
 ...
 do {
  ...
  if($taskText -match 'BADMINTON_RACKET_POSE_TEST PASS' -and $taskText -match 'BADMINTON_TIMING_CONTROL_RESTORED'){
   if($WheelAngles){Write-Output 'Wheel face -30 / 0 / +30 screenshots saved; manual control restored'}else{Write-Output 'Left / overhead / right screenshots saved; hit ray matches racket; manual control restored'}
   return
  }

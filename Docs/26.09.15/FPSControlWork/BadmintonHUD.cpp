...
void ABadmintonHUD::DrawHUD()
{
	...
	DrawAimPreview();
	Label(TEXT("SHUTTLE / FIRST PERSON"), 24, 24, 1.25f, Accent, 300);
	Label(TEXT("AI PRACTICE   /   FIRST TO 11"), 24, 49, .9f, Muted, 300);
	...
	{
		Label(bServing ? TEXT("YOUR SERVE - SELECT 1 / 2 / 3 / 4, THEN SPACE") : TEXT("AI SERVE - GET READY"), Center, 99, 1.1f, Accent, 730, true);
	}
	...
	for (int32 Index=0; Index<4; ++Index)
	{
		...
		Label(FString::Printf(TEXT("%d  %s"),Index+1,Badminton::ShotLabel(Shots[Index])),X+9,Height-74,.92f,Color,134);
		Label(Data->Get(Shots[Index]).StaminaCost>0 ? FString::Printf(TEXT("%.0f STAMINA"),Data->Get(Shots[Index]).StaminaCost) : TEXT("FREE"),X+9,Height-53,.75f,Muted,134);
	}
	if (bArmed)
	{
		const float Y=Height-188, BarX=Center-220, BarWidth=440;
		Panel(Center-242,Y-27,484,107,PanelColor);
		Label(FString::Printf(TEXT("%s  /  SPACE TO HIT"),Badminton::ShotLabel(Controller->GetSelectedShot())),Center,Y-16,1.1f,Accent,450,true);
		...
		Label(Controller->IsRacketAligned() ? TEXT("RACKET ALIGNED   /   HIT THE GREEN ZONE") : TEXT("MOUSE - ALIGN WHITE RACKET WITH SHUTTLE"),Center,Y+52,.88f,Controller->IsRacketAligned()?Accent:Warning,455,true);
	}
	else if (bPlay)
	{
		Label(TEXT("MOUSE - PLACE SHOT   /   1-4 - LOCK SHOT"),Center,Height-145,1.05f,FLinearColor::White,730,true);
	}
	...
	Label(TEXT("WASD MOVE   |   SHIFT DASH   |   RMB CANCEL   |   MMB RESET AIM"),Center,Height-22,.82f,Muted,800,true);
}
...
void ABadmintonHUD::DrawAimPreview()
{
	...
	if (PlayerOwner->ProjectWorldLocationToScreen(Target, Screen))
	{
		...
		const TCHAR* Name = Controller->IsTimingArmed() ? TEXT("LOCKED") : TEXT("AIM");

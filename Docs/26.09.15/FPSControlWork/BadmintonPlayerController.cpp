...
FRotator ABadmintonPlayerController::GetCourtViewRotation() const
{
	...
	if (TimingShuttle && TimingShuttle->GetFlight().bFlying) { Focus = TimingShuttle->GetActorLocation(); }
...
void ABadmintonPlayerController::UpdateTiming()
{
	...
	if (RacketPreview)
	{
		RacketPreview->SetActorHiddenInGame(!bTimingArmed);
		if (bTimingArmed)
		{
			int32 Width, Height;
			GetViewportSize(Width, Height);
			...
			{
				RacketPreview->SetActorLocationAndRotation(GetEyePosition() + Direction * 100.f, PlayerCameraManager->GetCameraRotation());
			}
		}
	}
}
...
void ABadmintonPlayerController::RunTimingProbe(float DeltaTime)
{
	...
	if (TimingProbeStage==31 && TimingProbeShots>=5)
	{

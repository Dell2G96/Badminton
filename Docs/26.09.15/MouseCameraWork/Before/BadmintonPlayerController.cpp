...
{
	if (!bTimingArmed) { ClientTimingResult(TEXT("CHOOSE 1 / 2 / 3 / SPACE FIRST")); return; }
	int32 Width, Height;
...
void ABadmintonPlayerController::RunTimingProbe(float DeltaTime)
{
	...
	if (TimingProbeStage==31 && TimingProbeShots>=6 && QuickReceiveProbeMask==3 && bTimingProbeMissChecked)
	{
		...
		UE_LOG(LogTemp,Display,TEXT("BADMINTON_TIMING_TEST PASS mask=%d contacts=%d firstperson=OK aimlock=OK leftclick=OK spaceReceive=OK quickreceive=early+late"),TimingProbeStage,TimingProbeShots);
		bProbeReported=true;
		return;
	}

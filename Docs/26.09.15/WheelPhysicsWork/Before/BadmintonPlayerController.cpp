...
void ABadmintonPlayerController::BadmintonSelectDrop() { SelectStroke(EBadmintonShot::Drop); }
void ABadmintonPlayerController::BadmintonSelectSmash() { SelectStroke(EBadmintonShot::Smash); SelectTimedShot(EBadmintonShot::Smash); }
void ABadmintonPlayerController::BadmintonSelectClear() { SelectStroke(EBadmintonShot::Clear); }
void ABadmintonPlayerController::BadmintonSelectReceive() { SelectStroke(EBadmintonShot::Receive); }
void ABadmintonPlayerController::BadmintonHairpin() { SelectStroke(EBadmintonShot::Hairpin); }
...
void ABadmintonPlayerController::ServerSelectStroke_Implementation(EBadmintonShot Shot)
{
	...
	AimPreviewShot = Shot;
	bTimingArmed = false;
	ClientTimingResult(FString::Printf(TEXT("%s SELECTED / LEFT CLICK TO HIT"), Badminton::ShotLabel(Shot)));
}
...
void ABadmintonPlayerController::ServerInstantShot_Implementation(EBadmintonShot Shot, FVector Direction, FVector2D Aim)
{
	...
	{ ClientTimingResult(TEXT("LEFT CLICK TO SERVE")); return; }
...
void ABadmintonPlayerController::ServerArmTiming_Implementation(EBadmintonShot Shot, FVector2D Aim)
{
	...
	if (Shot != EBadmintonShot::Smash || Match->Phase != EBadmintonPhase::Rally) { ClientTimingResult(TEXT("LEFT CLICK TO SERVE")); return; }
	if (!TimingShuttle) { for (TActorIterator<ABadmintonShuttle> It(GetWorld()); It; ++It) { TimingShuttle = *It; break; } }
	if (!TimingShuttle) { return; }
...
void ABadmintonPlayerController::BadmintonStrike()
{
	...
	if (SelectedStroke != EBadmintonShot::Smash || bServe)
	{
		...
		ServerInstantShot(bServe ? EBadmintonShot::Clear : SelectedStroke, GetCourtViewRotation().Vector(), ShotAim);
	}
	else
	{
		if (!bTimingArmed) { ClientTimingResult(TEXT("PRESS 2 TO PREPARE SMASH / THEN LEFT CLICK")); return; }
...
void ABadmintonPlayerController::RunTimingProbe(float DeltaTime)
{
	...
	{
		UE_LOG(LogTemp, Display, TEXT("BADMINTON_TIMING_TEST PASS mask=63 contacts=%d Q=select E=select R=hairpinSelect space=receiveSelect allHits=leftclick selectionPersists=OK smash=timing aimlock=OK repeatGuard=OK racketRay=OK"), TimingProbeShots);
		bProbeReported = true;
		return;
	}
	...
	if (SelectedStroke != Wanted) { Fail(TEXT("stroke selection lost after hit")); return; }
	...
	UE_LOG(LogTemp, Display, TEXT("BADMINTON_KEY_SHOT_TEST PASS shot=%d selectedKey=%s leftclick=1 persistent=1 noTiming=%d"), static_cast<int32>(Wanted), *Key.ToString(), Wanted != EBadmintonShot::Smash);
}

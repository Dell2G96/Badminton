...
void ABadmintonHUD::DrawHUD()
{
	...
	const FString ContactText = bQuickReceive && bCanHit ? TEXT("SPACE RECEIVE READY") : Badminton::ContactHintText(Controller->GetContactHint());
	...
	{
		Label(bServing ? TEXT("YOUR SERVE - PRESS E") : TEXT("AI SERVE - GET READY"), Center, 99, 1.1f, Accent, 730, true);
	}
	...
	for (int32 Index=0; Index<5; ++Index)
	{
		...
		Label(Shots[Index] == EBadmintonShot::Smash ? TEXT("TIMING + LEFT CLICK") : TEXT("PRESS KEY TO HIT"),X+9,Height-53,.75f,Muted,134);
	}
	if (bQuickReceive)
	{
		const float Y = Height - 188;
		Panel(Center-242, Y-27, 484, 107, PanelColor);
		Label(TEXT("Q / E / R / SPACE - INSTANT HIT"), Center, Y-16, 1.1f, Accent, 450, true);
		Label(TEXT("MOVE RACKET WITH MOUSE - PRESS SHOT KEY"), Center, Y+17, 1.05f, FLinearColor::White, 455, true);
	...
	{
		Label(TEXT("MOUSE: FREE RACKET   |   E: SERVE   |   2 + CLICK: SMASH"),Center,Height-145,1.05f,FLinearColor::White,730,true);
	}

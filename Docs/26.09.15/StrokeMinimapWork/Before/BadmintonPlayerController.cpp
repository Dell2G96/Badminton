...
void ABadmintonPlayerController::CalcCamera(float DeltaTime, FMinimalViewInfo& OutResult)
{
	...
	if (RacketPreview)
	{
		...
		RacketPreview->SetActorLocationAndRotation(OutResult.Location + GetCourtViewRotation().Vector() * (100.f + (bSwing ? 12.f * Recoil : 0.f)),
			Badminton::FreeRacketRotation(ViewSide, CameraLook) + (bSwing ? FRotator(8.f * Recoil, -22.f * Recoil, 24.f * Recoil) : FRotator::ZeroRotator));
	}
}
...
bool ABadmintonPlayerController::GetAimPreview(FVector& Target, EBadmintonShot& Shot) const
{
	...
	Target = Shot == EBadmintonShot::Smash ? Badminton::PlacementTarget(State->CourtSide, Match->GetScore(State->CourtSide), false, LockedAim)
...
void ABadmintonPlayerController::BadmintonSelectDrop() { InstantShot(EBadmintonShot::Drop); }
void ABadmintonPlayerController::BadmintonSelectSmash() { SelectTimedShot(EBadmintonShot::Smash); }
void ABadmintonPlayerController::BadmintonSelectClear() { InstantShot(EBadmintonShot::Clear); }
void ABadmintonPlayerController::BadmintonSelectReceive() { InstantShot(EBadmintonShot::Receive); }
void ABadmintonPlayerController::BadmintonHairpin() { InstantShot(EBadmintonShot::Hairpin); }

void ABadmintonPlayerController::InstantShot(EBadmintonShot Shot)
{
	if (!IsLocalController()) { return; }
	// Instant keys replace any prepared smash and use the current racket direction.
	bTimingArmed = false;
	ApplyMouseLook(0.f, 0.f);
	ServerInstantShot(Shot, GetCourtViewRotation().Vector(), ShotAim);
	RacketSwingUntil = GetWorld()->GetTimeSeconds() + .22;
}

void ABadmintonPlayerController::ServerInstantShot_Implementation(EBadmintonShot Shot, FVector Direction, FVector2D Aim)
{
	bTimingArmed = false;
	const auto* Match = GetWorld()->GetGameState<ABadmintonGameState>();
	const auto* State = GetPlayerState<ABadmintonPlayerState>();
	...
	{ ClientTimingResult(TEXT("PRESS E TO SERVE")); return; }
...
void ABadmintonPlayerController::ServerArmTiming_Implementation(EBadmintonShot Shot, FVector2D Aim)
{
	...
	if (Shot != EBadmintonShot::Smash || Match->Phase != EBadmintonPhase::Rally) { ClientTimingResult(TEXT("PRESS E TO SERVE")); return; }
	if (!TimingShuttle) { for (TActorIterator<ABadmintonShuttle> It(GetWorld()); It; ++It) { TimingShuttle = *It; break; } }
	if (!TimingShuttle) { return; }
	...
	{ ClientTimingResult(TEXT("LOW STAMINA - USE E OR SPACE")); return; }
...
{
	if (!bTimingArmed) { ClientTimingResult(TEXT("2 + CLICK: SMASH / Q E R SPACE: INSTANT")); return; }
...
void ABadmintonPlayerController::ServerStrikeTiming_Implementation(FVector Direction)
{
	...
	const double Now = Match->GetServerWorldTimeSeconds();
	if (TimingShot == EBadmintonShot::Smash && bContactForecast && Now > TimingPerfectAt + .30) { ClientTimingResult(TEXT("TOO LATE")); return; }
	...
	ClientTimingResult(bContact && TimingShot != EBadmintonShot::Smash ? FString::Printf(TEXT("%s - CONNECTED"), Badminton::ShotLabel(TimingShot)) : bContact ? FString::Printf(TEXT("%s  |  POWER %d%%"), Grade, FMath::RoundToInt(100.f / Badminton::TimingFlightScale(DispatchQuality)))
...
void ABadmintonPlayerController::RunTimingProbe(float DeltaTime)
{
	...
	if (FParse::Param(FCommandLine::Get(), TEXT("BadmintonRacketPoseTest")))
	{
		...
		if (Index >= 3)
		{
			UE_LOG(LogTemp, Display, TEXT("BADMINTON_RACKET_POSE_TEST PASS left=OK overhead=OK right=OK rollSpan=180"));
			bProbeReported = true;
			return;
		}
		if (Match->Phase == EBadmintonPhase::WaitingForReady) { ServerSetReady(true); }
		const FVector2D Poses[] = {FVector2D(-90.,-12.), FVector2D(0.,85.), FVector2D(90.,-12.)};
	...
	{
		UE_LOG(LogTemp, Display, TEXT("BADMINTON_TIMING_TEST PASS mask=63 contacts=%d Q=instant E=instant R=hairpin space=instant smash=timing aimlock=OK repeatGuard=OK racketRay=OK"), TimingProbeShots);
		bProbeReported = true;
		return;
	}
	...
	else
	{
		...
		if (Wanted == EBadmintonShot::Receive && !bTimingProbeMissChecked)
		{
			...
			if (Flight.ShotSequence != Before || bTimingArmed) { Fail(TEXT("misaligned receive accepted")); return; }
	...
	Press(Key);
	if (Flight.ShotSequence != Before + 1 || Flight.Shot != Wanted || bTimingArmed) { Fail(TEXT("shot key did not strike exactly once")); return; }
	...
	UE_LOG(LogTemp, Display, TEXT("BADMINTON_KEY_SHOT_TEST PASS shot=%d key=%s instant=%d"), static_cast<int32>(Wanted), *Key.ToString(), Wanted != EBadmintonShot::Smash);
}

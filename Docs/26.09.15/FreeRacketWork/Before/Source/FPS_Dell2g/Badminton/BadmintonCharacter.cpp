...
{
	PrototypeBody->SetVisibility(!(IsPlayerControlled() && IsLocallyControlled()), true);
	if (UAbilitySystemComponent* ASC = GetAbilitySystemComponent())
	{
		ASC->InitAbilityActorInfo(GetPlayerState(), this);
		if (HasAuthority())
		{
			const TSubclassOf<UGameplayAbility> Classes[] = { UBadmintonClearAbility::StaticClass(), UBadmintonServeAbility::StaticClass(),
				UBadmintonDropAbility::StaticClass(), UBadmintonSmashAbility::StaticClass(), UBadmintonReceiveAbility::StaticClass(), UBadmintonDashAbility::StaticClass() };

...
void ABadmintonHUD::DrawHUD()
{
	...
	const bool bQuickReceive = !bArmed && Match->Phase == EBadmintonPhase::Rally;
	const bool bCanHit = Controller->GetContactHint() == Badminton::EContactHint::Ready;
	const FString ContactText = bQuickReceive && bCanHit ? TEXT("IN REACH + ALIGNED / SPACE TO HIT") : Badminton::ContactHintText(Controller->GetContactHint());
	...
	{
		Label(bServing ? TEXT("YOUR SERVE - SELECT 1 / 2 / 3, THEN LEFT CLICK") : TEXT("AI SERVE - GET READY"), Center, 99, 1.1f, Accent, 730, true);
	}
	...
	for (int32 Index=0; Index<4; ++Index)
	{
		...
		Label(Data->Get(Shots[Index]).StaminaCost>0 ? FString::Printf(TEXT("%.0f STAMINA"),Data->Get(Shots[Index]).StaminaCost) : Shots[Index] == EBadmintonShot::Receive ? TEXT("SPACE = INSTANT HIT") : TEXT("FREE"),X+9,Height-53,.75f,Muted,134);
	}
	if (bQuickReceive)
	{
		const float Y = Height - 188;
		Panel(Center-242, Y-27, 484, 107, PanelColor);
		Label(TEXT("QUICK RECEIVE / SPACE TO HIT"), Center, Y-16, 1.1f, Accent, 450, true);
		Label(TEXT("AIM RACKET WITH MOUSE + PRESS SPACE"), Center, Y+17, 1.05f, FLinearColor::White, 455, true);
	...
	{
		Label(TEXT("MOUSE LOOK / AIM   |   1-3 SHOTS   |   SPACE INSTANT RECEIVE"),Center,Height-145,1.05f,FLinearColor::White,730,true);
	}

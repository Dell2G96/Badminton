...
void ABadmintonPlayerController::CalcCamera(float DeltaTime, FMinimalViewInfo& OutResult)
{
	...
	OutResult.Rotation = GetCourtViewRotation();
	OutResult.FOV = 95.f;
	...
	// Update the viewmodel from the very same camera pose to avoid a frame of racket lag.
	if (RacketPreview)
	{
		...
		RacketPreview->SetActorLocationAndRotation(OutResult.Location + OutResult.Rotation.Vector() * (100.f + (bSwing ? 12.f * Recoil : 0.f)),
			OutResult.Rotation + (bSwing ? FRotator(8.f * Recoil, -22.f * Recoil, 0.f) : FRotator::ZeroRotator));
	}
}
...
void ABadmintonPlayerController::RestoreTimingProbeControl()
{
	...
	const bool bRestored = !IsMouseAimOverridden()
		...
		&& !bTimingArmed && FMath::IsNearlyEqual(UGameplayStatics::GetGlobalTimeDilation(this), 1.f);
	if (!bRestored)
	{
	...
	ClientTimingResult(TEXT("TEST FINISHED / MOUSE CONTROL RESTORED"));
}
...
void ABadmintonPlayerController::BadmintonSelectReceive()
{
	if (!IsLocalController()) { return; }
	// Space is a complete strike. Drop any prepared shot and use the current mouse view.
	bTimingArmed = false;
	ApplyMouseLook(0.f, 0.f);
	ServerQuickReceive(GetCourtViewRotation().Vector(), ShotAim);
	RacketSwingUntil = GetWorld()->GetTimeSeconds() + .22;
}

void ABadmintonPlayerController::ServerQuickReceive_Implementation(FVector Direction, FVector2D Aim)
{
	bTimingArmed = false;
	const auto* Match = GetWorld()->GetGameState<ABadmintonGameState>();
	const auto* State = GetPlayerState<ABadmintonPlayerState>();
	...
	{ ClientTimingResult(TEXT("RECEIVE DURING RALLY / SERVE WITH 1-3 + CLICK")); return; }
	...
	if (!Flight.bFlying || Flight.RallyId != Match->RallyId || Flight.LastHitterSide == State->CourtSide)
	...
	TimingShot = EBadmintonShot::Receive;
	LockedAim = Aim;
...
void ABadmintonPlayerController::ServerArmTiming_Implementation(EBadmintonShot Shot, FVector2D Aim)
{
	...
	{ ClientTimingResult(TEXT("LOW STAMINA - USE 3 OR SPACE")); return; }
...
{
	if (!bTimingArmed) { ClientTimingResult(TEXT("1-3 THEN CLICK / SPACE TO RECEIVE NOW")); return; }
	// The racket centre stays on the crosshair; the camera supplies its world hit direction.
...
void ABadmintonPlayerController::ServerStrikeTiming_Implementation(FVector Direction)
{
	...
	const double Now = Match->GetServerWorldTimeSeconds();
	if (TimingShot != EBadmintonShot::Receive && bContactForecast && Now > TimingPerfectAt + .30) { ClientTimingResult(TEXT("TOO LATE")); return; }
	...
	DispatchQuality = bQuickReceive ? .65f : bContactForecast ? Badminton::TimingQuality(Now - TimingPerfectAt) : .5f;
...
void ABadmintonPlayerController::UpdateTiming()
{
	...
	{
		const bool bExpired = TimingShot != EBadmintonShot::Receive && bContactForecast && Match->GetServerWorldTimeSeconds() > TimingPerfectAt + .30;
	...
	{
		const bool bSwing = GetWorld()->GetTimeSeconds() < RacketSwingUntil;
		const bool bReceivePreview = Match && Match->Phase == EBadmintonPhase::Rally;
...
void ABadmintonPlayerController::RunTimingProbe(float DeltaTime)
{
	...
	if (TimingProbeStage==31 && TimingProbeShots>=6 && QuickReceiveProbeMask==3 && bTimingProbeMissChecked)
	{
		...
		UE_LOG(LogTemp,Display,TEXT("BADMINTON_TIMING_TEST PASS mask=%d contacts=%d firstperson=OK aimlock=OK leftclick=OK spaceReceive=instant quickreceive=noClick+noGauge+20deg+override+repeatGuard mouseCamera=OK contactReady=OK practiceEvents=OK"),TimingProbeStage,TimingProbeShots);
		bProbeReported=true;
		return;
	}
	...
	if (bReceive)
	{
		// Stand behind the incoming shuttle so the test does not require looking past the camera limits.
	...
	const double Now = Match->GetServerWorldTimeSeconds();
	const bool bQuick = bReceive && (TimingProbeStage & 14) == 14;
	if (bQuick)
	{
		if (!bQuickReceiveProbeCaptured)
		{
		...
		const bool bReach = Flight.bLegalCrossing && Flight.Velocity.Z < 0.f
		...
		if (!bReach) { return; }
		if (!bTimingProbeMissChecked)
		{
			const FVector2D BeforeLook = CameraLook;
			ApplyMouseLook(-100000.f, 0.f);
			const int32 Before = TimingShuttle->GetFlight().ShotSequence;
			Press(EKeys::SpaceBar);
			if (Before != TimingShuttle->GetFlight().ShotSequence || bTimingArmed) { Fail(TEXT("misaligned space accepted or left armed")); return; }
			CameraLook = BeforeLook;
			bTimingProbeMissChecked = true;
		}
		// 16 degrees is outside regular shot tolerance and inside the receive's 20 degrees.
		const FVector ToShuttle = (TimingShuttle->GetActorLocation() - GetEyePosition()).GetSafeNormal();
		const FRotator Desired = ToShuttle.RotateAngleAxis(-16.f, FVector::CrossProduct(ToShuttle, FVector::UpVector).GetSafeNormal()).Rotation();
		ApplyMouseLook((FMath::FindDeltaAngleDegrees(State->CourtSide == 1 ? 180. : 0., Desired.Yaw) - CameraLook.X) / MouseLookSensitivity,
			(Desired.Pitch - CameraLook.Y) / MouseLookSensitivity);
		if (Badminton::RacketAimMatches(EBadmintonShot::Clear, GetCourtViewRotation().Vector(), ToShuttle))
		{ Fail(TEXT("receive aim-offset inside normal tolerance")); return; }
		if (GetContactHint() != Badminton::EContactHint::Ready) { return; }
		if (!bContactReadyProbeCaptured)
		{
			UpdateTiming();
		...
		// First receive has no selection; second replaces a prepared regular shot with an obsolete deadline.
		if (QuickReceiveProbeMask != 0) { BadmintonSelectClear(); }
		TimingPerfectAt = Now + (QuickReceiveProbeMask == 0 ? 2. : -2.);
		const int32 Before = TimingShuttle->GetFlight().ShotSequence;
		const int32 PreviousPracticeHits = Practice.RallyHits;
		Press(EKeys::SpaceBar);
		if (TimingShuttle->GetFlight().ShotSequence != Before + 1 || bTimingArmed
			|| TimingShuttle->GetFlight().Shot != EBadmintonShot::Receive || !FMath::IsNearlyEqual(DispatchQuality, .65f))
		{ Fail(TEXT("single space did not immediately receive without timing")); return; }
		if (Practice.RallyHits != PreviousPracticeHits + 1) { Fail(TEXT("receive practice event missing")); return; }
		Press(EKeys::SpaceBar);
		Click();
		if (TimingShuttle->GetFlight().ShotSequence != Before + 1 || bTimingArmed)
		{ Fail(TEXT("repeat space or leftover click produced extra shot")); return; }
		QuickReceiveProbeMask |= QuickReceiveProbeMask == 0 ? 1 : 2;
		UE_LOG(LogTemp, Display, TEXT("BADMINTON_QUICK_RECEIVE_TEST PASS mask=%d instant=1 noClick=1 noGauge=1 quality=%.2f"), QuickReceiveProbeMask, DispatchQuality);
		return;
	}
	if (!bTimingArmed)
	{
	...
	if (bContactForecast && Now >= TimingPerfectAt)
	{
		if (GetContactHint() == Badminton::EContactHint::AimMiss) { return; }
		if (GetContactHint() != Badminton::EContactHint::Ready) { Fail(TEXT("predicted click not ready")); return; }
		const int32 Before = TimingShuttle->GetFlight().ShotSequence;
		const int32 PreviousPracticeHits = Practice.RallyHits;
		Click();
		if (TimingShuttle->GetFlight().ShotSequence == Before) { Fail(TEXT("green racket did not produce actual contact")); return; }
		if (!bServe && Practice.RallyHits != PreviousPracticeHits + 1) { Fail(TEXT("practice challenge contact missing")); return; }
	}
}

...
Badminton::EContactHint ABadmintonPlayerController::EvaluateContact(const FVector& Direction) const
{
	...
	const EBadmintonShot ContactShot = bTimingArmed ? TimingShot : EBadmintonShot::Receive;
	...
	if (ContactShot != EBadmintonShot::Receive && bContactForecast && Match->GetServerWorldTimeSeconds() > TimingPerfectAt + .30) { return EContactHint::TooLate; }

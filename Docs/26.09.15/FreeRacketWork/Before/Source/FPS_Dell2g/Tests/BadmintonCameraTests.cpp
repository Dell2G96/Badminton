...
bool FBadmintonCameraLookTest::RunTest(const FString& Parameters)
{
	...
	TestTrue(TEXT("Upper right stops at limits"), Badminton::ApplyMouseLook(Initial, FVector2D(1e6, 1e6), .72f).Equals(FVector2D(65., 75.)));
	TestTrue(TEXT("Lower left stops at limits"), Badminton::ApplyMouseLook(Initial, FVector2D(-1e6, -1e6), .72f).Equals(FVector2D(-65., -35.)));
...
bool FBadmintonCameraAimTest::RunTest(const FString& Parameters)
{
	...
	for (double X : {-65.,0.,65.}) for (double Y : {-35.,-12.,0.,75.})
	{

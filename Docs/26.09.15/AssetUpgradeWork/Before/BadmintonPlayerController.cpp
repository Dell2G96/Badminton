...
void ABadmintonPlayerController::CalcCamera(float DeltaTime, FMinimalViewInfo& OutResult)
{
	...
	if (RacketPreview)
	{
		...
		RacketPreview->SetActorLocationAndRotation(OutResult.Location + OutResult.Rotation.Vector() * (100.f + (bSwing ? 20.f * Swing : 0.f)),
			OutResult.Rotation + (bSwing ? FRotator(15.f * Swing, -45.f * Swing, 0.f) : FRotator::ZeroRotator));
	}
}
...
bool ABadmintonPlayerController::IsRacketAligned() const
{
	...
	return FVector::DotProduct(GetCourtViewRotation().Vector(), (TimingShuttle->GetActorLocation() - GetEyePosition()).GetSafeNormal()) >= FMath::Cos(FMath::DegreesToRadians(12.f));
}
...
void ABadmintonPlayerController::RunTimingProbe(float DeltaTime)
{
	...
	if (TimingProbeStage==31 && TimingProbeShots>=6 && QuickReceiveProbeMask==3 && bTimingProbeMissChecked)
	{
		...
		UE_LOG(LogTemp,Display,TEXT("BADMINTON_TIMING_TEST PASS mask=%d contacts=%d firstperson=OK aimlock=OK leftclick=OK spaceReceive=OK quickreceive=early+late mouseCamera=OK"),TimingProbeStage,TimingProbeShots);
		bProbeReported=true;
		return;
	}

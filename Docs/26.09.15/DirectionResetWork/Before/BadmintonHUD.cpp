...
void ABadmintonHUD::DrawHUD()
{
	...
	const FString ContactText = bSelectedStroke && bCanHit ? TEXT("IN REACH + ALIGNED / LEFT CLICK TO HIT") : Badminton::ContactHintText(Controller->GetContactHint());
	...
	Label(Controller->GetSelectedShot() == EBadmintonShot::Smash ? TEXT("FACE FIXED / SMASH") : FString::Printf(TEXT("FACE %+0.0f deg / MOUSE WHEEL"), Controller->GetRacketTilt()), 24, 88, .85f, Accent, 300);
	...
	{
		Label(bServing ? TEXT("YOUR SERVE - LEFT CLICK") : TEXT("AI SERVE - GET READY"), Center, 99, 1.1f, Accent, 730, true);
	}
	...
	for (int32 Index=0; Index<5; ++Index)
	{
		...
		Label(Shots[Index] == EBadmintonShot::Smash ? TEXT("TIMING + LEFT CLICK") : TEXT("SELECT / LEFT CLICK"),X+9,Height-53,.75f,Muted,134);
	}
	if (bSelectedStroke)
	{
		const float Y = Height - 188;
		Panel(Center-242, Y-27, 484, 107, PanelColor);
		Label(Controller->GetSelectedShot() == EBadmintonShot::Smash ? TEXT("SMASH / PRESS 2 TO PREPARE") : FString::Printf(TEXT("%s / LEFT CLICK TO HIT"), Badminton::ShotLabel(Controller->GetSelectedShot())), Center, Y-16, 1.1f, Accent, 450, true);
		Label(TEXT("Q / E / R / SPACE: SELECT STROKE"), Center, Y+17, 1.05f, FLinearColor::White, 455, true);
	...
	{
		Label(TEXT("MOUSE: FREE RACKET   |   LEFT CLICK: SERVE   |   2 + CLICK: SMASH"),Center,Height-145,1.05f,FLinearColor::White,730,true);
	}
	...
	Label(TEXT("WASD MOVE | SHIFT DASH | WHEEL: FACE ANGLE | RMB CANCEL | MMB: RESET VIEW + FACE"),Center,Height-22,.82f,Muted,800,true);
}
...
void ABadmintonHUD::DrawMinimap(const ABadmintonGameState* Match, int32 LocalSide, float Width)
{
	...
	Label(TEXT("COURT MAP"), X + 98.5f, Y + 11.f, .95f, FLinearColor::White, 175.f, true);
	...
	Panel(X+13,Y+293,6,6,Accent); Label(TEXT("YOU"),X+24,Y+290,.75f,Accent,65);
	Label(TEXT("AI"),X+112,Y+290,.75f,OpponentColor,65); Ring(FVector2D(X+102,Y+296),3.f,OpponentColor);
	Ring(FVector2D(X+16,Y+317),3.f,Warning); Label(TEXT("SHUTTLE"),X+24,Y+312,.75f,Warning,72);
	Ring(FVector2D(X+102,Y+317),4.f,LandingColor); Label(TEXT("LANDING"),X+112,Y+312,.75f,LandingColor,77);

...
void ABadmintonPlayerController::RunTimingProbe(float DeltaTime)
{
	...
	if (TimingProbeStage==31 && TimingProbeShots>=6 && QuickReceiveProbeMask==3 && bTimingProbeMissChecked)
	{
		...
		UE_LOG(LogTemp,Display,TEXT("BADMINTON_TIMING_TEST PASS mask=%d contacts=%d firstperson=OK aimlock=OK leftclick=OK spaceReceive=OK quickreceive=early+late+20deg mouseCamera=OK contactReady=OK practiceEvents=OK"),TimingProbeStage,TimingProbeShots);
		bProbeReported=true;
		return;
	}
	...
	if (bReceive)
	{
		...
		const double Facing = State->CourtSide == 1 ? 180. : 0.;
		const double DesiredYaw = FMath::FindDeltaAngleDegrees(Facing, Desired.Yaw);
		ApplyMouseLook((DesiredYaw - CameraLook.X) / MouseLookSensitivity, (Desired.Pitch - CameraLook.Y) / MouseLookSensitivity);
	}
	...
	if (bQuick ? bQuickReach : bContactForecast && Now >= TimingPerfectAt)
	{
		...
		else
		{
			if (bQuick)
			{
				// Add 16 degrees in world space: outside normal aim tolerance, inside receive tolerance.
				const FVector ToShuttle = (TimingShuttle->GetActorLocation() - GetEyePosition()).GetSafeNormal();
				const FVector OffsetDirection = ToShuttle.RotateAngleAxis(-16.f, FVector::CrossProduct(ToShuttle, FVector::UpVector).GetSafeNormal());
				const FRotator Desired = OffsetDirection.Rotation();
				...
				{ Fail(TEXT("receive aim-offset test did not leave normal tolerance")); return; }
			}
			// Aim remains an independent requirement; a moving target may leave the limited camera view.
			if (GetContactHint() == Badminton::EContactHint::AimMiss) { return; }
			if (GetContactHint() != Badminton::EContactHint::Ready)
			{
				UE_LOG(LogTemp, Error, TEXT("BADMINTON_TIMING_TEST FAIL predicted click not ready: %s"), Badminton::ContactHintText(GetContactHint()));
				bProbeReported = true;
				return;
			}
			if (bQuick && !bContactReadyProbeCaptured)
			{
				UpdateTiming();
			...
			if (!bServe && Practice.RallyHits != PreviousPracticeHits + 1) { Fail(TEXT("confirmed contact missing from practice challenge")); return; }
			if (bQuick)
			{

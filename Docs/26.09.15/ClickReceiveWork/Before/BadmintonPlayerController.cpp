...
void ABadmintonPlayerController::SetupInputComponent()
{
	...
	InputComponent->BindKey(EKeys::SpaceBar, IE_Pressed, this, &ThisClass::BadmintonStrike);
...
{
	const ABadmintonGameState* Match = GetWorld()->GetGameState<ABadmintonGameState>();
	if (!Match || Match->RallyId != RallyId || Match->GetServerWorldTimeSeconds() > PerfectAt + .30) { return; }
	bTimingArmed = true;
	TimingShot = Shot;
	LockedAim = Aim;
	TimingRally = RallyId;
...
void ABadmintonPlayerController::ServerStrikeTiming_Implementation(FVector Direction)
{
	...
	const double Now = Match->GetServerWorldTimeSeconds();
	if (Now > TimingPerfectAt + .30) { ClientTimingResult(TEXT("TOO LATE")); return; }
	DispatchQuality = Badminton::TimingQuality(Now - TimingPerfectAt);
	...
	ClientTimingResult(bContact ? FString::Printf(TEXT("%s  |  POWER %d%%"), Grade, FMath::RoundToInt(100.f / Badminton::TimingFlightScale(DispatchQuality)))
...
void ABadmintonPlayerController::UpdateTiming()
{
	...
	{
		const bool bExpired = Match->GetServerWorldTimeSeconds() > TimingPerfectAt + .30;
...
void ABadmintonPlayerController::RunTimingProbe(float DeltaTime)
{
	...
	if (TimingProbeStage==31 && TimingProbeShots>=5 && bTimingProbeMissChecked)
	{
		if (GetEyePosition().Z-GetPawn()->GetActorLocation().Z<70.f) { Fail(TEXT("first-person eye")); return; }
		UE_LOG(LogTemp,Display,TEXT("BADMINTON_TIMING_TEST PASS mask=%d contacts=%d firstperson=OK aimlock=OK"),TimingProbeStage,TimingProbeShots);
		bProbeReported=true;
		return;
	}
	...
	if (Match->GetServerWorldTimeSeconds()>=TimingPerfectAt)
	{
		int32 Width, Height;
		GetViewportSize(Width,Height);
		FVector2D Screen;
		...
		if (TimingProbeStage == 31 && !bTimingProbeMissChecked)
		{
			...
			UE_LOG(LogTemp, Display, TEXT("BADMINTON_TIMING_MISS_TEST PASS alignment and repeated Space"));
		}
		else { BadmintonStrike(); }
	}
}

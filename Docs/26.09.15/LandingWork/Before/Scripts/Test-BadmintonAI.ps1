...
$taskArgs = $taskPrefix + @('/Game/Badminton/Maps/L_Badminton_Prototype', '-game', '-BadmintonAITest', '-unattended', '-nosplash',
...
try {
    ...
    do {
        ...
        if ($taskText -match 'BADMINTON_AI_TEST FAIL|Fatal error:|Assertion failed:') { throw "AI 검증 실패: $taskLog" }

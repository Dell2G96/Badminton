...
void ABadmintonHUD::DrawHUD()
{
	...
	if (bPlay)
	{
		...
		const FLinearColor Crosshair = bArmed && Controller->IsRacketAligned() ? Accent : FLinearColor::White;
	...
	if (bQuickReceive)
	{
		...
		Label(Controller->IsRacketAligned() ? TEXT("ALIGNED - CLICK WHEN WITHIN REACH") : TEXT("MOUSE LOOK - FOLLOW THE SHUTTLE"), Center, Y+52, .88f, Controller->IsRacketAligned()?Accent:Warning, 455, true);
	}
	else if (bArmed)
	{
		...
		Label(Controller->IsRacketAligned() ? TEXT("RACKET ALIGNED   /   HIT THE GREEN ZONE") : TEXT("MOUSE LOOK - CENTRE RACKET ON SHUTTLE"),Center,Y+52,.88f,Controller->IsRacketAligned()?Accent:Warning,455,true);
	}
	else if (bPlay)
	{

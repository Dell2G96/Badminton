...
void ABadmintonGameMode::SpawnSoloOpponent()
{
	...
	State->SetPlayerName(TEXT("Practice AI"));
	State->bReady = true;
...
bool ABadmintonGameMode::TryShot(ABadmintonCharacter* Player, int32 RequestedRallyId, EBadmintonShot Shot, const FVector2D& Aim)
{
	...
	else if (State->Phase == EBadmintonPhase::Rally)
	{
		...
		if (FVector::Dist2D(PlayerPosition, HitPosition) > Params.Reach) { return Result(TEXT("OutOfReach")); }
		if (HitPosition.Z < Params.MinimumHeight) { return Result(TEXT("TooLow")); }
		if (HitPosition.Z > Params.MaximumHeight) { return Result(TEXT("TooHigh")); }
		if (HitPosition.X * Badminton::ForwardSign(Side) > 0.f) { return Result(TEXT("OpponentHalf")); }
	}
	else
	{
	...
	if (Controller && Controller->ResolveTimedContact(Target, Quality, bAligned))
	{
		if (!bAligned) { return Result(TEXT("RacketAimMiss")); }
		if (Shot == EBadmintonShot::Receive)
		{
			// A steady defensive return has no timing scatter or power bonus.
			FlightTime = FMath::Max(1.35f, Params.FlightTime);
		}
		else
		{
			Target = Badminton::ScatterTarget(Target, Side, bServe, Quality, State->RallyId * 7919 + Shuttle->GetFlight().ShotSequence);
			FlightTime = (Shot == EBadmintonShot::Clear ? 2.1f : Params.FlightTime) * Badminton::TimingFlightScale(Quality);
		}
		// Keep the intended landing point when increasing power; ensure the arc can clear the net.
		for (int32 Attempt = 0; Attempt < 60; ++Attempt)
		{
			FVector Position = HitPosition;
			FVector Velocity = ABadmintonShuttle::SolveVelocity(HitPosition, Target, FlightTime);
			const double Decay = 1. + .15 * HitPosition.X / Velocity.X;

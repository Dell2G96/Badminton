...
void ABadmintonHUD::DrawHUD()
{
	...
	{
		Label(bServing ? TEXT("YOUR SERVE - SELECT 1 / 2 / 3 / 4, THEN LEFT CLICK") : TEXT("AI SERVE - GET READY"), Center, 99, 1.1f, Accent, 730, true);
	}
	...
	if (bQuickReceive)
	{
		...
		Label(TEXT("ALIGN WHITE RACKET + LEFT CLICK"), Center, Y+17, 1.05f, FLinearColor::White, 455, true);
		Label(Controller->IsRacketAligned() ? TEXT("ALIGNED - CLICK WHEN WITHIN REACH") : TEXT("MOUSE - FOLLOW THE SHUTTLE"), Center, Y+52, .88f, Controller->IsRacketAligned()?Accent:Warning, 455, true);
	}
	else if (bArmed)
	{

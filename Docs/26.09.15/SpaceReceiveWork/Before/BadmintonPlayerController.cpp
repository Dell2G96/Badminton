...
void ABadmintonPlayerController::BeginPlay()
{
	...
	if (IsLocalController())
	{
		...
		bTimingProbe = FParse::Param(FCommandLine::Get(), TEXT("BadmintonTimingTest"));
...
void ABadmintonPlayerController::SetupInputComponent()
{
	...
	InputComponent->BindKey(EKeys::One, IE_Pressed, this, &ThisClass::BadmintonSelectDrop);
	InputComponent->BindKey(EKeys::Two, IE_Pressed, this, &ThisClass::BadmintonSelectSmash);
	InputComponent->BindKey(EKeys::Three, IE_Pressed, this, &ThisClass::BadmintonSelectClear);
	InputComponent->BindKey(EKeys::Four, IE_Pressed, this, &ThisClass::BadmintonSelectReceive);
...
void ABadmintonPlayerController::CalcCamera(float DeltaTime, FMinimalViewInfo& OutResult)
{
	...
	const FRotator Desired = GetCourtViewRotation();
	FirstPersonRotation = FMath::RInterpTo(FirstPersonRotation, Desired, DeltaTime, 9.f);
	OutResult.Rotation = FirstPersonRotation;
	OutResult.FOV = 95.f;
}
...
void ABadmintonPlayerController::SendShot(EBadmintonShot Shot)
{
	...
	if (const ABadmintonPlayerState* State = GetPlayerState<ABadmintonPlayerState>(); State && Match)
	{
		...
		Payload.EventTag = Shot == EBadmintonShot::Drop ? TAG_BadmintonDropEvent : Shot == EBadmintonShot::Smash ? TAG_BadmintonSmashEvent
			: Shot == EBadmintonShot::Serve ? TAG_BadmintonServeEvent : Shot == EBadmintonShot::Receive ? TAG_BadmintonReceiveEvent : TAG_BadmintonClearEvent;
...
void ABadmintonPlayerController::UpdateMouseAim()
{
	...
	if (bNetworkProbe || bShotProbe || bMatchProbe || bAbilityProbe || bAIProbe || bTimingProbe) { return; }
	...
	if (FMath::IsFinite(DeltaX) && FMath::IsFinite(DeltaY))
	{
		if (bTimingArmed)
		{
			RacketScreen.X = FMath::Clamp(RacketScreen.X + DeltaX * .0015, .08, .92);
			RacketScreen.Y = FMath::Clamp(RacketScreen.Y - DeltaY * .0015, .12, .82);
			return;
		}
...
bool ABadmintonPlayerController::GetAimPreview(FVector& Target, EBadmintonShot& Shot) const
{
	...
	Target = Badminton::PlacementTarget(State->CourtSide, Match->GetScore(State->CourtSide), Shot == EBadmintonShot::Serve, bTimingArmed ? LockedAim : ShotAim);
...
void ABadmintonPlayerController::PlayerTick(float DeltaTime)
{
	...
	if (bTimingProbe && IsLocalController() && !bProbeReported) { RunTimingProbe(DeltaTime); }
#endif
}
...
FRotator ABadmintonPlayerController::GetCourtViewRotation() const
{
	...
	FVector Focus(Sign * 340.f, 0, 220.f);
	// Keep the court visible while choosing placement; track the incoming shuttle once a shot is armed.
	if (bTimingArmed && TimingShuttle && TimingShuttle->GetFlight().bFlying) { Focus = TimingShuttle->GetActorLocation(); }
	FRotator View = (Focus - GetEyePosition()).Rotation();
	const float Facing = Sign > 0.f ? 0.f : 180.f;
	View.Yaw = Facing + FMath::Clamp(FMath::FindDeltaAngleDegrees(Facing, View.Yaw), -65.f, 65.f);
	View.Pitch = FMath::Clamp(View.Pitch, -32.f, 70.f);
	View.Roll = 0.f;
	return View;
}

void ABadmintonPlayerController::BadmintonSelectDrop() { SelectTimedShot(EBadmintonShot::Drop); }
void ABadmintonPlayerController::BadmintonSelectSmash() { SelectTimedShot(EBadmintonShot::Smash); }
void ABadmintonPlayerController::BadmintonSelectClear() { SelectTimedShot(EBadmintonShot::Clear); }
void ABadmintonPlayerController::BadmintonSelectReceive() { SelectTimedShot(EBadmintonShot::Receive); }
...
void ABadmintonPlayerController::ServerArmTiming_Implementation(EBadmintonShot Shot, FVector2D Aim)
{
	...
	if (Shot != EBadmintonShot::Drop && Shot != EBadmintonShot::Smash && Shot != EBadmintonShot::Clear && Shot != EBadmintonShot::Receive) { return; }
	...
	{ ClientTimingResult(TEXT("WAIT FOR THE RETURN")); return; }
	...
	{ ClientTimingResult(TEXT("RECOVERING")); return; }
	if (State->GetAttributes()->GetStamina() < Data->Get(Shot).StaminaCost)
	{ ClientTimingResult(TEXT("LOW STAMINA - USE 3 OR 4")); return; }
	...
	if (!bServe && Shot != EBadmintonShot::Receive)
	{
		...
		UntilContact = 0.f;
		const float Height = Shot == EBadmintonShot::Smash ? 265.f : 235.f;
		for (int32 Step = 0; Step < 300; ++Step)
		{
			if (Position.X * Badminton::ForwardSign(State->CourtSide) <= 0.f && Velocity.Z < 0.f && Position.Z <= Height) { break; }
			if (Position.Z < Data->Get(Shot).MinimumHeight) { break; }
			ABadmintonShuttle::AdvanceFlight(Position, Velocity, .01f);
			UntilContact += .01f;
		}
		UntilContact = FMath::Max(.06f, UntilContact);
	}
	bTimingArmed = true;
	TimingShot = Shot;
	LockedAim = Aim;
...
{
	if (!bTimingArmed) { ClientTimingResult(TEXT("CHOOSE 1 / 2 / 3 / 4 FIRST")); return; }
	int32 Width, Height;
	GetViewportSize(Width, Height);
	FVector Origin, Direction;
	if (Width < 1 || Height < 1 || !DeprojectScreenPositionToWorld(RacketScreen.X * Width, RacketScreen.Y * Height, Origin, Direction)) { return; }
	// Standalone calls the authoritative method synchronously; remote worlds use the RPC.
	ServerStrikeTiming(Direction);
...
void ABadmintonPlayerController::ServerStrikeTiming_Implementation(FVector Direction)
{
	...
	if (Match->RallyId != TimingRally || Flight.ShotSequence != TimingSequence
		|| (Match->Phase != EBadmintonPhase::ReadyToServe && Match->Phase != EBadmintonPhase::Rally)) { ClientTimingResult(TEXT("RALLY CHANGED")); return; }
	const double Now = Match->GetServerWorldTimeSeconds();
	if (TimingShot != EBadmintonShot::Receive && Now > TimingPerfectAt + .30) { ClientTimingResult(TEXT("TOO LATE")); return; }
	const bool bQuickReceive = TimingShot == EBadmintonShot::Receive;
	DispatchQuality = bQuickReceive ? .65f : Badminton::TimingQuality(Now - TimingPerfectAt);
	const bool bServe = TimingShot == EBadmintonShot::Serve;
	bTimedAligned = bServe || FVector::DotProduct(Direction, (TimingShuttle->GetActorLocation() - GetEyePosition()).GetSafeNormal()) >= FMath::Cos(FMath::DegreesToRadians(12.f));
	DispatchTarget = Badminton::PlacementTarget(State->CourtSide, Match->GetScore(State->CourtSide), bServe, LockedAim);
	ShotAim = LockedAim;
	bTimedDispatch = true;
	...
	const TCHAR* Grade = DispatchQuality >= .99f ? TEXT("PERFECT") : DispatchQuality >= .6f ? TEXT("GOOD") : TEXT("WEAK");
	ClientTimingResult(bContact && bQuickReceive ? TEXT("QUICK RECEIVE - NICE RETURN") : bContact ? FString::Printf(TEXT("%s  |  POWER %d%%"), Grade, FMath::RoundToInt(100.f / Badminton::TimingFlightScale(DispatchQuality)))
		: bTimedAligned ? TEXT("MISS - CHECK REACH / HEIGHT") : TEXT("MISS - ALIGN THE WHITE RACKET"));
...
bool ABadmintonPlayerController::IsRacketAligned() const
{
	...
	GetViewportSize(Width, Height);
	FVector Origin, Direction;
	if (!DeprojectScreenPositionToWorld(RacketScreen.X * Width, RacketScreen.Y * Height, Origin, Direction)) { return false; }
	return FVector::DotProduct(Direction, (TimingShuttle->GetActorLocation() - GetEyePosition()).GetSafeNormal()) >= FMath::Cos(FMath::DegreesToRadians(12.f));
}
...
void ABadmintonPlayerController::UpdateTiming()
{
	...
	{
		const bool bExpired = TimingShot != EBadmintonShot::Receive && Match->GetServerWorldTimeSeconds() > TimingPerfectAt + .30;
		if (bExpired || Match->RallyId != TimingRally || !TimingShuttle || TimingShuttle->GetFlight().ShotSequence != TimingSequence
			|| (Match->Phase != EBadmintonPhase::ReadyToServe && Match->Phase != EBadmintonPhase::Rally))
		{
			BadmintonCancelShot();
			if (bExpired) { ClientTimingResult(TEXT("TOO LATE - SELECT AGAIN")); }
		}
	}
	...
	if (RacketPreview)
	{
		...
		RacketPreview->SetActorHiddenInGame(!bTimingArmed && !bSwing);
		if (bTimingArmed || bSwing)
		{
			int32 Width, Height;
			GetViewportSize(Width, Height);
			FVector Origin, Direction;
			if (DeprojectScreenPositionToWorld(RacketScreen.X * Width, RacketScreen.Y * Height, Origin, Direction))
			{
				const float Swing = bSwing ? 1.f - static_cast<float>((RacketSwingUntil - GetWorld()->GetTimeSeconds()) / .22) : 0.f;
				RacketPreview->SetActorLocationAndRotation(GetEyePosition() + Direction * (100.f + 20.f * Swing), PlayerCameraManager->GetCameraRotation() + FRotator(15.f * Swing, -45.f * Swing, 0));
			}
		}
	}
}
...
void ABadmintonPlayerController::RunTimingProbe(float DeltaTime)
{
	...
	auto Click = [this, &Fail]()
	{
		for (const FInputKeyBinding& Binding : InputComponent->KeyBindings)
		{
			if (Binding.Chord.Key == EKeys::SpaceBar) { Fail(TEXT("obsolete Space binding")); return; }
		}
		for (FInputKeyBinding& Binding : InputComponent->KeyBindings)
		{
			...
			{ Binding.KeyDelegate.Execute(EKeys::LeftMouseButton); return; }
		}
		Fail(TEXT("left click binding missing"));
	};
	...
	// The four real shot types plus serve must have passed the normal GAS/contact path.
	if (TimingProbeStage==31 && TimingProbeShots>=6 && QuickReceiveProbeMask==3 && bTimingProbeMissChecked)
	{
		if (GetEyePosition().Z-GetPawn()->GetActorLocation().Z<70.f) { Fail(TEXT("first-person eye")); return; }
		UE_LOG(LogTemp,Display,TEXT("BADMINTON_TIMING_TEST PASS mask=%d contacts=%d firstperson=OK aimlock=OK leftclick=OK quickreceive=early+late"),TimingProbeStage,TimingProbeShots);
		bProbeReported=true;
		return;
	}
	...
	if (!bTimingArmed)
	{
		ShotAim=FVector2D(.25,.4);
		if (bServe) { BadmintonSelectClear(); }
		else if (!(TimingProbeStage & (1<<static_cast<int32>(EBadmintonShot::Drop)))) { BadmintonSelectDrop(); }
		else if (!(TimingProbeStage & (1<<static_cast<int32>(EBadmintonShot::Smash)))) { BadmintonSelectSmash(); }
		else if (!(TimingProbeStage & (1<<static_cast<int32>(EBadmintonShot::Clear)))) { BadmintonSelectClear(); }
		else { BadmintonSelectReceive(); }
		return;
	}
	if (!LockedAim.Equals(FVector2D(.25,.4),.001)) { Fail(TEXT("aim lock changed")); return; }
	if (!bTimingProbeCaptured && TimingProbeShots >= 1 && GetTimingProgress()>.3f)
	{
	...
	const double Now = Match->GetServerWorldTimeSeconds();
	const auto& ReceiveParams = Match->GetShotData()->Get(EBadmintonShot::Receive);
	const FVector ShuttlePosition = TimingShuttle->GetActorLocation();
	const bool bQuick = TimingShot == EBadmintonShot::Receive;
	if (bQuick && !bQuickReceiveProbeCaptured)
	{
		FScreenshotRequest::RequestScreenshot(FPaths::ProjectSavedDir()/TEXT("Screenshots/BadmintonQuickReceive.png"),true,false);
		bQuickReceiveProbeCaptured = true;
		return;
	}
	const bool bQuickReach = bReceive && Flight.bLegalCrossing && Flight.Velocity.Z < 0.f
		&& ShuttlePosition.X * Badminton::ForwardSign(State->CourtSide) <= 0.f
		&& ShuttlePosition.Z >= ReceiveParams.MinimumHeight && ShuttlePosition.Z <= ReceiveParams.MaximumHeight
		&& FVector::Dist2D(GetPawn()->GetActorLocation(), ShuttlePosition) < ReceiveParams.Reach - 10.f;
	if (bQuick ? bQuickReach : Now >= TimingPerfectAt)
	{
		// A receive must work even when a hypothetical timing deadline is far in the past or future.
		if (bQuick)
		{
			TimingPerfectAt = Now + (QuickReceiveProbeMask == 0 ? 2. : -2.);
			UpdateTiming();
			if (!bTimingArmed) { Fail(TEXT("quick receive expired by timing")); return; }
		}
		int32 Width, Height;
		GetViewportSize(Width,Height);
		FVector2D Screen;
		if (!bServe)
		{
			if (!ProjectWorldLocationToScreen(TimingShuttle->GetActorLocation(),Screen) || Width<1 || Height<1) { Fail(TEXT("shuttle not visible")); return; }
			RacketScreen=FVector2D(Screen.X/Width,Screen.Y/Height);
		}
		if (TimingProbeStage == 31 && QuickReceiveProbeMask == 3 && !bTimingProbeMissChecked)
		{
			RacketScreen = FVector2D(.03, .03);
			const int32 Before = TimingShuttle->GetFlight().ShotSequence;
			Click();
			Click();
			if (Before != TimingShuttle->GetFlight().ShotSequence || bTimingArmed) { Fail(TEXT("misaligned or repeated strike accepted")); return; }
			bTimingProbeMissChecked = true;
			UE_LOG(LogTemp, Display, TEXT("BADMINTON_TIMING_MISS_TEST PASS alignment and repeated click"));
		}
		else
		{
			const int32 Before = TimingShuttle->GetFlight().ShotSequence;
			...
			if (bQuick)
			{
				if (TimingShuttle->GetFlight().ShotSequence == Before || !FMath::IsNearlyEqual(DispatchQuality, .65f))
				{ Fail(TEXT("quick receive depends on timing")); return; }
				QuickReceiveProbeMask |= QuickReceiveProbeMask == 0 ? 1 : 2;
				UE_LOG(LogTemp, Display, TEXT("BADMINTON_QUICK_RECEIVE_TEST PASS mask=%d quality=%.2f"), QuickReceiveProbeMask, DispatchQuality);
			}
		}
	}
}

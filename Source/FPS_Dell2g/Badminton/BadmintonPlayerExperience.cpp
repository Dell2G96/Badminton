...
Badminton::EContactHint ABadmintonPlayerController::EvaluateContact(const FVector& Direction) const
{
	if (bThirdPersonControl)
	{
		...
		FVector Position;
		return GetThirdPersonContact(Quality, Seconds, Position);
	}
	...
	const EBadmintonShot ContactShot = Match && Match->Phase == EBadmintonPhase::ReadyToServe ? EBadmintonShot::Serve : bTimingArmed ? TimingShot : SelectedStroke;
	if (ContactShot == EBadmintonShot::Smash && !bTimingArmed) { return EContactHint::SelectShot; }
	if (!Match || !State || !GetPawn() || !TimingShuttle || !State->bReady || Match->ConnectedPlayers != 2
		|| (bTimingArmed && (Match->RallyId != TimingRally || TimingShuttle->GetFlight().ShotSequence != TimingSequence))) { return EContactHint::WaitReturn; }
	...
	const auto& Params = Match->GetShotData()->Get(ContactShot);
	if (State->GetAttributes()->GetStamina() < Params.StaminaCost) { return EContactHint::LowStamina; }
	if (ContactShot == EBadmintonShot::Smash && bContactForecast && Match->GetServerWorldTimeSeconds() > TimingPerfectAt + .30) { return EContactHint::TooLate; }
	if (Match->Phase == EBadmintonPhase::ReadyToServe && ContactShot == EBadmintonShot::Serve && Match->ServingSide == State->CourtSide)
	{
	...
	return RacketAimMatches(ContactShot, Direction, TimingShuttle->GetActorLocation() - GetEyePosition()) ? EContactHint::Ready : EContactHint::AimMiss;
}
...
{
	if (!bTimingArmed || TimingShot != EBadmintonShot::Smash || !TimingShuttle || !GetPawn()) { return; }
	const auto* Match = GetWorld()->GetGameState<ABadmintonGameState>();
	const auto* State = GetPlayerState<ABadmintonPlayerState>();
...
void ABadmintonPlayerController::BadmintonSensitivity(float Value)
{
	...
	ClientTimingResult(FString::Printf(TEXT("마우스 감도 %.2f"), MouseLookSensitivity));
}
...
void ABadmintonPlayerController::BadmintonDifficulty(int32 Level)
{
	...
	const auto* Match = GetWorld()->GetGameState<ABadmintonGameState>();
	if (Match && Match->Phase == EBadmintonPhase::Rally) { ClientTimingResult(TEXT("랠리가 끝난 뒤 난이도를 변경하세요")); return; }
	AIDifficulty = FMath::Clamp(Level, 0, 2);
	SavePracticeSettings();
	ClientTimingResult(FString::Printf(TEXT("상대 난이도: %s"), AIDifficulty == 0 ? TEXT("쉬움") : AIDifficulty == 1 ? TEXT("보통") : TEXT("어려움")));
}
...
void ABadmintonPlayerController::ClientPracticeContact_Implementation(int32 RallyId, int32 Sequence, int32 Hitter, EBadmintonShot Shot)
{
	...
	if (Previous < 10 && Practice.RallyHits >= 10) { ChallengeMessage = TEXT("10회 연속 랠리 달성!"); ChallengeMessageUntil = GetWorld()->GetTimeSeconds() + 3.; }
}
void ABadmintonPlayerController::ClientPracticeLanding_Implementation(int32 RallyId, int32 Winner, int32 Hitter, EBadmintonShot Shot, FVector Position)
{
	...
	{
		ChallengeMessage = Practice.ComboPoints > Before ? TEXT("드롭 + 스매시 득점!") : TEXT("목표 명중! / 반대편 목표를 노리세요");

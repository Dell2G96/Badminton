...
{
	Serve = Clear;
	Hairpin.FlightTime = .95f;
	Hairpin.TargetDistance = 95.f;
	Hairpin.TargetHeight = 5.f;
	Hairpin.MinimumHeight = 45.f;
	Hairpin.MaximumHeight = 340.f;
	Hairpin.Reach = 195.f;
	Hairpin.Recovery = .28f;
	Hairpin.StaminaCost = 4.f;
...
bool UBadmintonShotData::ResolveAimTarget(EBadmintonShot Shot, int32 Side, int32 Score, const FVector2D& Aim, FVector& Target) const
{
	...
	const float Depth = FMath::Clamp(Params.TargetDistance + Aim.Y * FMath::Clamp(AimDepth, 0.f, 200.f),
		bServe ? Badminton::ShortServiceLine + Margin : Shot == EBadmintonShot::Hairpin ? 65.f : 120.f, Shot == EBadmintonShot::Hairpin ? 135.f : Badminton::HalfLength - 120.f);
...
const FBadmintonShotParameters& UBadmintonShotData::Get(EBadmintonShot Shot) const
{
	...
	case EBadmintonShot::Hairpin: return Hairpin;

...
struct FBadmintonFlightState
{
	...
	UPROPERTY() bool bHasLanding = false;
	UPROPERTY() FVector_NetQuantize100 Landing = FVector::ZeroVector;
};
...
public:
	...
	void Launch(const FVector& Position, const FVector& Target, int32 HitterSide, int32 RallyId, float Duration = 1.7f, EBadmintonShot Shot = EBadmintonShot::Clear, const FVector2D& Aim = FVector2D::ZeroVector, const FVector* LaunchVelocity = nullptr);
	void ResetForServe(const FVector& Position, int32 RallyId);
	const FBadmintonFlightState& GetFlight() const { return Flight; }
	FVector GetSimulationVelocity() const { return HasAuthority() ? SimVelocity : FVector(Flight.Velocity); }
	...
	// Forecast the untouched flight using the same sphere, collision responses and net reaction as Tick.
	static bool PredictLanding(UWorld* World, FVector Position, FVector Velocity, int32 HitterSide, bool bLegalCrossing, FVector& OutLanding, const AActor* IgnoreActor = nullptr);

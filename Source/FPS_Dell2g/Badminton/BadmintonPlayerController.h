...
public:
	...
	UFUNCTION(Exec) void BadmintonToggleDualView();
	bool IsDualViewEnabled() const { return !bThirdPersonControl && bDualView; }
	bool UsesThirdPersonControl() const { return bThirdPersonControl; }
	Badminton::EContactHint GetThirdPersonContact(float& Quality, float& Seconds, FVector& ContactPosition) const;
	Badminton::EContactHint GetThirdPersonContactForShot(EBadmintonShot Shot, float& Quality, float& Seconds, FVector& ContactPosition) const;
	void SampleThirdPersonFlight(FVector& Position, FVector& Velocity) const;
	class UTextureRenderTarget2D* GetThirdPersonTexture() const;
	bool ProjectThirdPerson(const FVector& Position, FVector2D& Screen) const;
	...
	EBadmintonShot GetSelectedShot() const;
	EBadmintonShot GetAutomaticDropShot() const;
	...
	float GetRacketTilt() const { return SelectedStroke == EBadmintonShot::Smash ? 0.f : RacketTilt; }
	float GetDispatchedRacketTilt() const { return bTimedDispatch && TimingShot != EBadmintonShot::Smash ? DispatchRacketTilt : 0.f; }
	FVector GetDispatchedFaceNormal() const { return DispatchFaceNormal; }
	...
	bool GetShotDirectionPreview(FVector& Origin, FVector& Velocity) const;
...
private:
	...
	float ThirdPersonProbeTime = 0.f;
	int32 ThirdPersonProbeShots = 0;
	int32 ThirdPersonProbeMask = 0;
	int32 ThirdPersonProbeSequence = 0;
	bool bThirdPersonProbeDone = false;
	bool bThirdPersonReadyCaptured = false;
	void UpdateDualView();
	void RunDualViewProbe(float DeltaTime);
	float DualViewProbeTime = 0.f;
	int32 DualViewProbeStage = 0;
	bool bDualView = true;
	UPROPERTY(Transient) TObjectPtr<class USceneCaptureComponent2D> ThirdPersonCapture;
	UPROPERTY(Transient) TObjectPtr<class UTextureRenderTarget2D> ThirdPersonTexture;
	...
	UFUNCTION(Server, Reliable) void ServerInstantShot(EBadmintonShot Shot, FVector Direction, FVector2D Aim, float FaceTilt);
	void RacketWheelUp();
	void RacketWheelDown();
	void AdjustRacketFace(float Steps);
	float RacketTilt = 0.f;
	float DispatchRacketTilt = 0.f;
	FVector DispatchFaceNormal = FVector::ForwardVector;
	bool bWheelInputProbeChecked = false;
	void SelectStroke(EBadmintonShot Shot);
	UFUNCTION(Server, Reliable) void ServerSelectStroke(EBadmintonShot Shot);
	EBadmintonShot SelectedStroke = EBadmintonShot::Receive;
	UFUNCTION(Client, Reliable) void ClientResetStroke();

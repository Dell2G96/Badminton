...
bool FBadmintonRacketPhysicsTest::RunTest(const FString& Parameters)
{
	...
	TestTrue(TEXT("Positive wheel sends this return right"),ResolveRacketImpact(Incoming,Desired,Face,12,EBadmintonShot::Clear).Y>Desired.Y);
	TestTrue(TEXT("Negative wheel sends this return left"),ResolveRacketImpact(Incoming,Desired,Face,-12,EBadmintonShot::Clear).Y<Desired.Y);
